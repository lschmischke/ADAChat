...
In diesem Abschnitt soll kurz und bündig beschrieben werden, wie das Programm von der Logik her arbeitet. Auf die Datenstrukturen die zur Informationshaltung und -verwaltung genutzt werden wird nicht näher eingegangen. 

Zu Beginn erzeugt und konfiguriert der Server, während des Elaboration Tasks, seinen Kommunikationssocket für eingehende Verbindungsanfragen auf der Transportschicht. Hierzu zählt beispielsweise das Konfigurieren von IP-Adresse und Port, sowie das Binden des Socket an sie. Daraufhin erzeugt und startet er einen weiteren Task, folgend Main-Task genannt. 

Der Main-Task hat die Aufgabe auf eingehende Verbindungsanfragen zu lauschen und eine Verbindung zwischen Client und Server auf der Tansportschicht herzustellen. Zum Lauschen auf den Socket benutzt er eine blockierende Methode, die zurückkehrt sobald Aktivität anliegt. Daraufhin erzeugt der Main-Task für diesen Client einen eigenen Socket und Task. Als letzte Aktion startet er den Client-Task noch und fährt nach dessen Aktivierungsphase mit einem erneuten Lauschen auf den Server-Socket fort. Solch ein Client-Task wird für jeden Nutzer bzw. Client erzeugt. 
...
\begin{minipage}{\linewidth}
	\centering
	\frame{\includegraphics[width=0.85\linewidth]{img/Client_Login.png}}
...
\begin{minipage}{\linewidth}
	\centering
	\frame{\includegraphics[width=1.0\linewidth]{img/client_chat.png}}

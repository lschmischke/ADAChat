...
Beim Aufbau von Datenstrukturen, wo die einen Bestandteil der anderen sind traten sehr oft Zirkuläre Abhängigkeitsprobleme auf. Da in Ada ein Identifier vor dessen Verwendung deklariert werden muss war es nicht einfach Protected Types, Types, Pointer Types, Interfaces frei miteinander zu kombinieren. 

Zudem zeigte sich, dass das Einbinden von Spezifikationsdateien (.ads) in eine Bodydatei (.adb) ein anderes Folgeverhalten aufweisen kann als wenn diese in die dem Body zugehörige Spezifikationsdatei eingebunden werden. Manche zirkuläre Abhängigkeit konnte dadurch aufgelöst werden.
